...
class PolarCamMainApp(QtWidgets.QWidget):
    ...
    def update_image(self, cv_img):
        ...
        if self.bCaptureImgFlag:
            ...
            #self.last_deglared_image = self.get_predictions_column_from_cndet( right_image[0:h//2, 0:w], right_image[h//2:h, 0:w])
    ...
    def addResultToXlsxEntry():
        """Adds a entry to the current xlsx session.
           Each row contains: datetime, original_image, text_area, deglared_image, text_area, dual_image, text_area
        """
        pass
    ...
    def clean_up_image(self):
        ...
        if self.last_saved_dual_image_filename != '':
            files = {
                'file': open(os.path.join(topSrcFolder, self.last_saved_dual_image_filename), 'rb')
            }
